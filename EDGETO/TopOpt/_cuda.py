...
class TopOpt:
    ...
    def FEA_integrals(self, rho):
        rho_ = self.material_model(rho, 1e6, plain=True)
        ...
        return strain, stress, strain_energy, von_mises
    ...
    def update_design(self, rho, df):
        ...
        """
        rho = rho.reshape(-1, 1)
        xU = np.clip(rho + self.move, 0., 1.0)
        xL = np.clip(rho - self.move, 0., 1.0)
        ...
        while np.any((l2 - l1) / (l2 + l1) > 1e-4):
            ...
            valids = self.material_model.evaluate_constraint(rho_new, self.mesh.As,self.mesh.volume, np=np)
