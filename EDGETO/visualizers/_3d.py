...
import numpy as np
import k3d
from ..geom import StructuredMesh3D, GeneralMesh, CuStructuredMesh3D, CuGeneralMesh
...
def plot_problem_3D(problem: Union[TopOpt, CuTopOpt], rho=None, face_color=0x888888, force_color=0xff8300, constraint_color=0xff6347):
    """Plot FEA problem with forces and boundary conditions"""
    # Extract mesh data
    if isinstance(problem.mesh, CuStructuredMesh3D):
        nodes = problem.mesh.nodes.get()
        elements = problem.mesh.elements.get()
    else:
        nodes = problem.mesh.nodes
        elements = problem.mesh.elements

    if isinstance(problem, CuTopOpt):
        f = problem.f.get().reshape(-1, problem.mesh.dof)
        c = problem.kernel.constraints.get().reshape(-1, problem.mesh.dof)
        approx_elem_size = (problem.mesh.As.get().max()/(4/3 * np.pi))**(1/3)
    else:
        f = problem.f.reshape(-1, problem.mesh.dof)
        c = problem.kernel.constraints.reshape(-1, problem.mesh.dof)
        approx_elem_size = (problem.mesh.As.max()/(4/3 * np.pi))**(1/3)
    
    if rho is not None:
        elements = elements[rho > 0.5]
    
    # sphere diameter as element size approximation
...
def plot_mesh_3D(mesh: Union[StructuredMesh3D, CuStructuredMesh3D, CuGeneralMesh], rho=None, face_color=0x888888):
    """Plot FEA problem with forces and boundary conditions"""
    # Extract mesh data
    if isinstance(mesh, CuStructuredMesh3D):
        nodes = mesh.nodes.get()
        elements = mesh.elements.get()
    else:
        nodes = mesh.nodes
...
def plot_field_3D(
    mesh: Union[StructuredMesh3D, GeneralMesh, CuStructuredMesh3D, CuGeneralMesh],
    field,
    rho=None,
    colormap='viridis',
    min_value=None,
