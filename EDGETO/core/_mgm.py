...
from ..kernels._processors import StructuredStiffnessKernel
        ...
        for d in range(dof):
            ...
            for di in range(-1, 2):
                ...
                for dj in range(-1, 2):
                    ...
                    # Only accumulate total weight once per coarse point
                    if d == 0:
                        total_weight += weight
            ...
            for ci in range(i_coarse, i_coarse + i % 2 + 1):
                ...
                for cj in range(j_coarse, j_coarse + j % 2 + 1):
                    ...
                    for ck in range(k_coarse, k_coarse + k % 2 + 1):
                        ...
                        if d == 0:  # Only count once per node
                            count += 1
...
def restriction_matrix_2d_(nel, dof, nnz_per_row, Cp, Cj, Cx):
    nx_fine, ny_fine = nel[0] + 1, nel[1] + 1
    nx_coarse = nel[0] // 2 + 1
    ny_coarse = nel[1] // 2 + 1
    n_coarse_nodes = nx_coarse * ny_coarse
    ...
    for idx_coarse in prange(n_coarse_nodes):
        ...
        row_start = Cp[idx_coarse * dof]
        counter = np.zeros(dof, dtype=np.int32)
        ...
        for d in range(dof):
            ...
            for di in range(-1, 2):
                ...
                for dj in range(-1, 2):
                    ...
                    # Store in CSR format
                    Cj[row_start + counter[d]] = idx_fine * dof + d
                    Cx[row_start + counter[d]] = weight
                    counter[d] += 1
            
            # Normalize weights
            if total_weight > 0:
                for k in range(counter[d]):
                    Cx[row_start + k] /= total_weight
            
            if d < dof - 1:
                row_start += nnz_per_row[idx_coarse * dof + d]

@njit(["void(i4[:], i4, i4[:], i4[:], i4[:], f4[:])", "void(i4[:], i4, i4[:], i4[:], i4[:], f8[:])"], cache=True, parallel=True)
def restriction_matrix_3d_(nel, dof, nnz_per_row, Cp, Cj, Cx):
    nx_fine, ny_fine, nz_fine = nel[0] + 1, nel[1] + 1, nel[2] + 1
    nx_coarse = nel[0] // 2 + 1
    ny_coarse = nel[1] // 2 + 1
    nz_coarse = nel[2] // 2 + 1
...
def get_restriction_matrix(nel, dof, dtype=np.float64):
    """Generate restriction matrix in CSR format"""
    if len(nel) == 2:
        nx_fine, ny_fine = nel[0] + 1, nel[1] + 1
        nx_coarse = nel[0] // 2 + 1
        ny_coarse = nel[1] // 2 + 1
        ...
        nnz_per_row = np.ones(n_coarse_nodes * dof, dtype=np.int32) * 9
        restriction_matrix_ = restriction_matrix_2d_
    else:
        nx_fine, ny_fine, nz_fine = nel[0] + 1, nel[1] + 1, nel[2] + 1
        nx_coarse = nel[0] // 2 + 1
        ny_coarse = nel[1] // 2 + 1
        ...
        nnz_per_row = np.ones(n_coarse_nodes * dof, dtype=np.int32) * 27
        restriction_matrix_ = restriction_matrix_3d_
    
    Cp = np.zeros(n_coarse_nodes * dof + 1, dtype=np.int32)
    Cp[1:] = np.cumsum(nnz_per_row)
    Cj = np.zeros(int(Cp[-1]), dtype=np.int32)
    Cx = np.zeros(int(Cp[-1]), dtype=dtype)
    
    restriction_matrix_(nel, dof, nnz_per_row, Cp, Cj, Cx)
    
    return csr_matrix((Cx, Cj, Cp), 
                     shape=(n_coarse_nodes * dof, n_fine_nodes * dof))

@njit(["void(i4[:], i4, i4[:], i4[:], i4[:], f4[:])", "void(i4[:], i4, i4[:], i4[:], i4[:], f8[:])"], cache=True, parallel=True)
def prolongation_matrix_2d_(nel, dof, nnz_per_row, Cp, Cj, Cx):
    nx_fine, ny_fine = nel[0] + 1, nel[1] + 1
    nx_coarse = nel[0] // 2 + 1
    ny_coarse = nel[1] // 2 + 1
    n_fine_nodes = nx_fine * ny_fine
        ...
        for d in range(dof):
            ...
            for ci in range(i_coarse, i_coarse + i % 2 + 1):
                ...
                for cj in range(j_coarse, j_coarse + j % 2 + 1):
                    ...
                    # Store in CSR format
                    Cj[row_start + counter[d]] = idx_coarse * dof + d
                    Cx[row_start + counter[d]] = 1.0/total_weight  # Will normalize later
                    counter[d] += 1
            
            # Normalize weights
            if count > 0:
                for k in range(counter[d]):
                    Cx[row_start + k] /= count
            
            if d < dof - 1:
                row_start += nnz_per_row[idx_fine * dof + d]

@njit(["void(i4[:], i4, i4[:], i4[:], i4[:], f4[:])", "void(i4[:], i4, i4[:], i4[:], i4[:], f8[:])"], cache=True, parallel=True)
def prolongation_matrix_3d_(nel, dof, nnz_per_row, Cp, Cj, Cx):
    nx_fine, ny_fine, nz_fine = nel[0] + 1, nel[1] + 1, nel[2] + 1
    nx_coarse = nel[0] // 2 + 1
    ny_coarse = nel[1] // 2 + 1
    nz_coarse = nel[2] // 2 + 1
...
def get_prolongation_matrix(nel, dof, dtype=np.float64):
    """Generate prolongation matrix in CSR format"""
    if len(nel) == 2:
        nx_fine, ny_fine = nel[0] + 1, nel[1] + 1
        nx_coarse = nel[0] // 2 + 1
        ny_coarse = nel[1] // 2 + 1
        ...
        nnz_per_row = np.ones(n_fine_nodes * dof, dtype=np.int32) * 4
        prolongation_matrix_ = prolongation_matrix_2d_
    else:
        nx_fine, ny_fine, nz_fine = nel[0] + 1, nel[1] + 1, nel[2] + 1
        nx_coarse = nel[0] // 2 + 1
        ny_coarse = nel[1] // 2 + 1
        ...
        nnz_per_row = np.ones(n_fine_nodes * dof, dtype=np.int32) * 8
        prolongation_matrix_ = prolongation_matrix_3d_
    
    Cp = np.zeros(n_fine_nodes * dof + 1, dtype=np.int32)
    Cp[1:] = np.cumsum(nnz_per_row)
    Cj = np.zeros(int(Cp[-1]), dtype=np.int32)
    Cx = np.zeros(int(Cp[-1]), dtype=dtype)
    
    prolongation_matrix_(nel, dof, nnz_per_row, Cp, Cj, Cx)
    
    return csr_matrix((Cx, Cj, Cp), 
                     shape=(n_fine_nodes * dof, n_coarse_nodes * dof), dtype=dtype)
