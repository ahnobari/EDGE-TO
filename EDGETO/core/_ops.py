...
def FEA_Integrals_node_basis_parallel(K_single, D_single, B_single, elements_flat, nel, weights, U, dof=3, elements_size=8, B_size = 6):
...
@njit(["Tuple((f4[:, :], f4[:, :], f4[:]))(f4[:, :, :], f4[:, :, :], f4[:, :, :], i4[:], i4, f4[:], f4[:], i4, i4, i4)",
       "Tuple((f8[:, :], f8[:, :], f8[:]))(f8[:, :, :], f8[:, :, :], f8[:, :, :], i4[:], i4, f8[:], f8[:], i4, i4, i4)"], cache=True, parallel=True)
def FEA_Integrals_node_basis_parallel_full(Ks, Ds, Bs, elements_flat, nel, weights, U, dof=3, elements_size=8, B_size = 6):
...
def FEA_Integrals_node_basis_parallel_flat(K_flat, D_flat, B_flat, elements_flat, elements_ptr, K_ptr, B_ptr, D_ptr, nel, weights, U, dof=3, B_size = 6):
