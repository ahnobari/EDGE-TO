...
from ._cuda import (
    ...
    csrcsr_matmat_kernel,
    csrcsr_matmat_nnz_per_row_kernel,
    FEA_Integrals_node_basis_parallel_full_cuda_kernel,
    FEA_Integrals_node_basis_parallel_flat_cuda_kernel,
    FEA_Integrals_node_basis_parallel_cuda_kernel,
)
...
def csrcsr_matmat_nnz_per_row_cuda(Ap, Aj, Bp, Bj, max_nnz, n_row, n_col):
    # Initialize output array
    nnz_per_row = cp.zeros(n_row, dtype=np.int32)
    
    # Launch parameters
    threadsperblock = 256
    blockspergrid = (n_row + (threadsperblock - 1)) // threadsperblock
    
    max_nnz = int(np.ceil(max_nnz/32))*32
    
    # Launch kernel
    csrcsr_matmat_nnz_per_row_kernel((blockspergrid,), (threadsperblock,),
                                        (Ap, Aj, Bp, Bj, n_row, n_col, nnz_per_row), max_nnz)
    
    return nnz_per_row
    
def csrcsr_matmat_cuda(A, B):
    n_col = B.shape[1]
    n_row = A.shape[0]
    Bp = B.indptr
    Bj = B.indices
    ...
    max_nnz = int(np.diff(Bp).max() * np.diff(Ap).max())
    
    nnz_per_row = csrcsr_matmat_nnz_per_row_cuda(Ap, Aj, Bp, Bj, max_nnz, n_row, n_col)
    
    # Launch parameters
    threadsperblock = 256
    blockspergrid = (n_row + (threadsperblock - 1)) // threadsperblock
    
    Cp = cp.zeros(n_row+1, dtype=cp.int32)
    Cp[1:] = cp.cumsum(cp.array(nnz_per_row))
    Cj = cp.zeros(int(Cp[-1]), dtype=cp.int32) - 1
    Cx = cp.zeros(int(Cp[-1]), dtype=A.dtype)

    # Launch kernel
    csrcsr_matmat_kernel((blockspergrid,), (threadsperblock,), (Ap, Aj, Ax, Bp, Bj, Bx, n_row, Cp, Cj, Cx))
    
    M = cp.sparse.csr_matrix((n_row, n_col))
    M.data = Cx
    M.indices = Cj
    M.indptr = Cp
    return M

def csrcsr_matmat_cuda_(A, B, Cp):
    n_col = B.shape[1]
    n_row = A.shape[0]
    Bp = B.indptr
    Bj = B.indices
...
def FEA_Integrals_node_basis_parallel_cuda(K_single, D_single, B_single, elements_flat, nel, weights, U, dof=3, elements_size=8, B_size=6):
    """CUDA version of FEA_Integrals_node_basis_parallel"""
    ...
    FEA_Integrals_node_basis_parallel_cuda_kernel((blockspergrid,), (threadsperblock,),
...
def FEA_Integrals_node_basis_parallel_full_cuda(Ks, Ds, Bs, elements_flat, nel, weights, U, dof=3, elements_size=8, B_size=6):
    """CUDA version of FEA_Integrals_node_basis_parallel_full"""
    ...
    # Call the CUDA kernel
    FEA_Integrals_node_basis_parallel_full_cuda_kernel((blockspergrid,), (threadsperblock,),
        (Ks, Ds, Bs, elements_flat, nel, weights, U,
         dof, elements_size, B_size, strain, stress, strain_energy))
    
    return strain, stress, strain_energy

def FEA_Integrals_node_basis_parallel_flat_cuda(K_flat, D_flat, B_flat, elements_flat, elements_ptr, K_ptr, B_ptr, D_ptr, nel, weights, U, dof=3, B_size=6):
    """CUDA version of FEA_Integrals_node_basis_parallel_flat"""
    ...
    # Call the CUDA kernel
    FEA_Integrals_node_basis_parallel_flat_cuda_kernel((blockspergrid,), (threadsperblock,),
